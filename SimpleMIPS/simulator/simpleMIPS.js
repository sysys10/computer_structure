    ...
    methods(Memory, {
      ...
      dump: function (start, nrow, ncol) {
        ...
        for (i = 0; i < n; i++) {
          ...
          result +=
            " " +
            padLeft(this.getByte(offset).toString(16), "0", 2) +
  ...
  var CPU = (function () {
    ...
    (function () {
      var cur,
        ...
        INST_ALL = [],
        INST_CAT = {
          // instruction categorized by assembly format
          RRR: [],
          RRI: [],
          RRA: [],
          //RRC : [],
        ...
        if (!buffer) {
          ...
          for (var i = 0; i < 32; i++) {
            str +=
              ...
              i +
              "\t: 0x" +
              padLeft(this.registerFile[i].toString(16), "0", 8) +
              "\n";
          }
          return str;
        } else {
        ...
        case 63: // simulator special
          switch (func) {
            ...
            case 1: // print memory
              this.eventBus.post(
                "print",
                "0x" + padLeft(r[rs].toString(16), "0", 8),
                "m",
                mem.getByte(r[rs])
              );
              break;
    ...
    function _aStep() {
      ...
      if (id_ex.oprASrc == ex_ma.regDst && ex_ma.regSrc == 0) {
      ...
      if (id_ex.oprBSrc == ex_ma.regDst && ex_ma.regSrc == 0) {
      ...
      switch (id_ex.aluOp) {
        ...
        case ALU_OP.ADD: // add
          tmp =
            (aluA & 0x80000000 ? 0x100000000 + aluA : aluA) +
      ...
      if (id_ex.regDst != 0) {
        ...
        if (id_ex.memOp > 0 && id_ex.memOp < 4) {
          ...
          // sI
          if (newOprASrc == id_ex.regDst || newOprBSrc == id_ex.regDst) {
            stopPCUpdate = true;
          }
        }
        // memVal depend
        // sII2, sIII2
        if (newMemOp > 3 && newMemSrc == id_ex.regDst) {
          //stopPCUpdate = true;
        }
        ...
        if (newMemOp > 3 && newMemSrc == curRegDstEXMA) {
      ...
      if (confirmBranch) {
        ...
        self.pc = self.pc + 4;
        if (self.pc > 0xffffffff) self.pc -= 0x100000000;
      } else {
        self.stalls++;
      }
      this.cycle++;
  ...
  var Assembler = (function () {
    ...
    var PI_TABLE = [
        {
          ...
          n: "la",
          e: [TOKEN_TYPES.REGOPR, TOKEN_TYPES.COMMA, TOKEN_TYPES.WORD],
          t: "lui $r1,__h16__{2} " + "ori {0},$r1,__l16__{2}",
        },
        {
          // load immediate : li $rn, imm32
          n: "li",
          e: [TOKEN_TYPES.REGOPR, TOKEN_TYPES.COMMA, TOKEN_TYPES.INTEGER],
          t: "lui $r1,{2.H} " + "ori {0},$r1,{2.L}",
        },
        {
          // push register : pushr $rn
          n: "pushr",
          e: [TOKEN_TYPES.REGOPR],
          t: "sw {0},0($sp)" + "addi $sp,$sp,-4",
        },
        {
          // pop to register : pushr $rn
          n: "popr",
          e: [TOKEN_TYPES.REGOPR],
          t: "lw {0},4($sp)" + "addi $sp,$sp,4",
        },
      ],
      PI_COUNT,
      PI_NAMES = [],
    ...
    function packString(str) {
      ...
      if (i == 0) {
        ...
        res.push(
          (str.charCodeAt(n - 2) * 16777216 + str.charCodeAt(n - 1)) << 16
        );
      } else {
        res.push(
          str.charCodeAt(n - 3) * 16777216 +
    ...
    function packIntegers(list, unitSize) {
      ...
      if (unitSize == 4) {
        ...
        for (i = 0; i < n; i += 2) {
          result.push(
            convertHalfword(list[i]) * 65536 + convertHalfword(list[i + 1])
          );
        }
      } else if (unitSize == 1) {
        n = list.length;
    ...
    function checkImmediateRange(imm, instName) {
      ...
      if (INST_IMM_SHIFT.indexOf(instName) >= 0) {
        ...
        if (INST_UNSIGNED.indexOf(instName) >= 0) {
          ...
          if (imm < 0 || imm > 65535) {
            throw new Error(
              "Unsigned integer " + imm + " out of range (0~65535)"
            );
            return false;
          }
        } else {
          ...
          if (imm < -32768 || imm > 32767) {
            throw new Error(
              "Signed integer " + imm + " out of range (-32768~32767)"
            );
            return false;
          }
        }
      ...
      switch (type) {
        ...
        case INST_TYPES.RRI: // e.g. addi rt, rs, imm
          ...
          if (expectedTokens) {
            ...
            throw new Error(
              "Expecting 2 register operands and 1 immediate for " + instName
            );
          }
          break;
        ...
        case INST_TYPES.RC: // e.g. lw rt, offset(base)
          ...
          if (expectedTokens) {
            ...
            if (tmp >= -32768 && tmp < 32768) {
              ...
              throw new Error(
                "Offset value " + tmp + " out of range (-32768~32767)."
              );
            }
          } else {
            throw new Error(
              "Expecting 1 register operand and 1 immediate for " + instName
            );
          }
          break;
        ...
        case INST_TYPES.R: // e.g. jr rs
          expectedTokens = tokenList.expect([TOKEN_TYPES.REGOPR]);
          if (expectedTokens) {
            result.rs = expectedTokens[0].value;
          } else {
      ...
      while (tokens.getLength() > 0) {
        ...
        if (curToken) {
          ...
          if (symbols[curToken.value]) {
            ...
            symbols[curToken.value] =
              status.section == "text"
                ? status.textCurrentAddr
                : status.dataCurrentAddr;
          }
    ...
    function assemble(src, config) {
      ...
      statusTable.dataStartAddr =
        config.dataStartAddr != undefined ? config.dataStartAddr : 0x10000000;
      statusTable.textStartAddr =
        config.textStartAddr != undefined ? config.textStartAddr : 0x00040000;
      ...
      for (i = 0; i < n; i++) {
        try {
          ...
          infoList.push.apply(
            infoList,
            parseLine(curTokenList, i + 1, symbolTable, statusTable)
          );
        } catch (err) {
      ...
      if (!(dEnd < tStart || dStart > tEnd)) {
        throw new Error(
          "Overlap detected between data section and text section."
        );
      }
      var dataMem = [],
        textMem = [];
